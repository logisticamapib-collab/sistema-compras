...
export default function EditarRequisicion({ requisicion, onVolver, onGuardado }) {
  ...
  const [form, setForm] = useState({
    ...
    criticidad: requisicion.criticidad,
  ...
  const cargarDatos = async () => {
    setLoading(true)
    const [{ data: a }, { data: cc }, { data: cg }, { data: p }, { data: l }] = await Promise.all([
      ...
      supabase.from('requisicion_lineas').select('*').eq('requisicion_id', requisicion.id)
    ])
    setArticulos(a || [])
    setCentrosCostos(cc || [])
    setCuentasGastos(cg || [])
    ...
    setLineas(l?.map(linea => ({
      id: linea.id,
      articulo_id: linea.articulo_id?.toString() || '',
      ...
      proveedor_sugerido_id: linea.proveedor_sugerido_id?.toString() || '',
      notas: linea.notas || ''
    })) || [])
    setLoading(false)
  }
  ...
  function lineaVacia() {
    return {
      articulo_id: '', descripcion_libre: '', cantidad: '',
      unidad_medida: 'PZA', centro_costo_id: '', cuenta_gasto_id: '',
      proveedor_sugerido_id: '', notas: ''
    }
  }
  ...
  const guardar = async (enviar = false) => {
    ...
    await supabase.from('requisicion_lineas').delete().eq('requisicion_id', requisicion.id)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.encabezado}>
        ...
        <h2 style={styles.titulo}>Editar borrador — {requisicion.folio}</h2>
      </div>
      ...
      <div style={styles.seccion}>
        ...
        <div style={styles.campo}>
          <label style={styles.label}>Justificacion {form.criticidad === 'alta' ? '*' : '(opcional)'}</label>
