...
export default function CuentasGastos() {
  ...
  const cargarDatos = async () => {
    ...
    const [{ data: r }, { data: s }] = await Promise.all([
      supabase.from('cuentas_gastos').select('*, sites(nombre)'),
  ...
  const guardar = async () => {
    ...
    let error
    if (editando) {
      const r = await supabase.from('cuentas_gastos').update(payload).eq('id', editando.id)
      error = r.error
    } else {
      const r = await supabase.from('cuentas_gastos').insert(payload)
      error = r.error
    }
    if (error) { setError(error.message); return }
    setExito(editando ? 'Cuenta de gastos actualizada' : 'Cuenta de gastos guardada')
  ...
  const toggleActivo = async (r) => {
    await supabase.from('cuentas_gastos').update({ activo: !r.activo }).eq('id', r.id)
    await cargarDatos()
  }
