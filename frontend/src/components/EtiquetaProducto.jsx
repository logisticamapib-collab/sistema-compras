...
export const CONFIG_DEFECTO = {
  ...
  tamanos: { numero_parte: 19, descripcion: 12, snp: 26, lado: 15, tipo: 21, lote: 11, fecha: 12, qr_in: 0.80 },
}
...
export default function EtiquetaProducto({ datos, config }) {
  ...
  const anchoQr = Number(tam.qr_in) || 0.80
...
function estilos(cfg, tam, anchoQr) {
  return {
    etiqueta: {
      ...
      padding: '0.07in 0.12in 0.09in', backgroundColor: '#fff', color: '#000',
    ...
    numeroParte: { fontSize: `${tam.numero_parte}pt`, fontWeight: '700', lineHeight: '1.0', letterSpacing: '-0.2pt' },
    descripcion: { fontSize: `${tam.descripcion}pt`, fontWeight: '700', lineHeight: '1.05' },
    cliente: { fontSize: `${Math.max(7, Number(tam.descripcion) - 4)}pt`, fontWeight: '600', lineHeight: '1.05' },
    ...
    rotuloSnp: { fontSize: `${tam.snp}pt`, fontWeight: '700', lineHeight: 1 },
    cantidad: { fontSize: `${tam.snp}pt`, fontWeight: '700', letterSpacing: '1.5pt', lineHeight: 1 },
    indicadores: { display: 'flex', flexDirection: 'column', alignItems: 'center', lineHeight: 1, marginRight: `${anchoQr + 0.12}in`, minWidth: '0.55in' },
    ...
    sello: { flex: 1, border: '1px solid #000', height: `${anchoQr}in`, fontSize: '8pt', padding: '2pt 3pt', boxSizing: 'border-box' },
